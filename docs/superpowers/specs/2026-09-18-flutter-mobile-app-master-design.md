...
| **S3** | **Direct-insert policies with no guard triggers** on money/result tables: `tournament_registrations` (`tr_own_insert`, `payment_status` defaults `'pending'` but is client-settable), `withdrawal_requests` (`wr_own_insert`), `match_results` (`mr_player_insert`, `mr_own_update_pending`), `friendly_matches` (`friendly_matches_challenger_insert` **and `friendly_matches_participant_or_staff_update` — either participant may UPDATE any column of their match, incl. `challenger_paid` / `status` / Paystack references, on ₦ stakes**; found by a corrected audit whose first pass had wrongly filtered out policies mentioning `is_staff()`), `friendly_match_results`. No `BEFORE INSERT/UPDATE` guard exists on any of them. | `pg_policies`, `pg_trigger` (only `marketplace_listings` and `buy_requests` have status guards) | Possible self-marking of a registration as paid, self-confirming a result, or inserting withdrawal rows — to be verified with the rolled-back role-switching script in the Phase 0A plan (no staging database — owner decision), not by exercising it on prod. |

**✅ Closed 2026-09-19.** S1, S2 and S3 (including the `friendly_matches` participant-UPDATE hole and the `match_results` update path) are fixed and verified on production — see `docs/superpowers/plans/2026-09-18-mobile-phase0a-security-hardening.md` in the web repo. Migration `20260918200000_lock_down_profiles_and_write_paths.sql` applied 2026-09-19; the rolled-back role-switching script (`supabase/verification/2026-09-18-lock-down.verify.sql`) read all-`VULNERABLE` pre-migration and all-`PASS` post-migration, and a full click-through smoke test (player + admin flows, incl. a live refund) passed with no `42501`s. **This is no longer a Phase 0 blocker.** The paragraphs below are kept as the historical record of the finding and are superseded where they describe future work.

**Required action (web repo) — original, historical.** *Review feedback (2026-09-18) incorporated:* **S1 is decoupled from the mobile timeline** — it is a live web issue (most players are minors) and is fixed on its own schedule, not as "Phase 0 of the app". S2/S3 remain Phase 0 exit criteria for mobile.
- **S1 — split in two, because a blanket revoke breaks the web app.** A grep of the web code shows other players' `whatsapp_number` is *intentionally* read in fixture coordination (`lib/dashboard/fixtures.ts`, `lib/matches/*-whatsapp.ts`), friendly match rooms (`dashboard/friendlies/[id]`), and admin bracket/matches/registrations/exchange pages (call sites to be confirmed one by one; the grep lists files, not contexts).
  - **Decided in plan `2026-09-18-mobile-phase0a-security-hardening.md` (supersedes the S1a/S1b split below).** Private columns are exactly `phone`, `whatsapp_number`, `notification_prefs`, `referred_by`, `deletion_requested_at`. **`kyc_verified` and `deleted_at` stay public on purpose** (verified badges and tombstones on rankings/exchange/hall of fame) and are protected from forgery by S2 instead. Own-row reads of private columns move to the service role (`.eq('id', user.id)` from the verified session — the existing wallet-page pattern); counterpart/staff reads use the service role after an explicit participant/staff check. Because column privileges are not row-aware, the fix is expand→contract: move code first, then replace table-level `SELECT` with a column allow-list. *(Original draft, superseded:)* column-level `REVOKE SELECT` on `phone`, `notification_prefs`, `referred_by`, `deletion_requested_at`, `deleted_at`, `kyc_verified`.
  - *(Superseded)* S1b proposed a `SECURITY DEFINER` counterpart accessor for `whatsapp_number`; the plan instead does the participant check in server code and reads with the service role (Match Room page), since the web reads a counterpart's number in exactly one player-facing place.
  - *(Corrected: no staging database exists — owner decision, §3.2.)* Both steps were instead verified via the rolled-back role-switching script plus expand-before-contract sequencing (code moved to the service role first, migration applied only after the expand step was live and smoke-tested), not a test database. Done 2026-09-19.
...
- **Options to get that database** — owner decision, listed in §17.1:
  ...
  **Whichever is chosen, a migration replay check is required before any Phase 2 write-path work** (owner decision 2026-09-18: **no staging database in Phase 0**; revisit before Phase 2 when real money enters) — build an empty database from `supabase/migrations/*.sql` and diff its schema against production. If it fails, fix or re-baseline (schema dump as a new baseline migration) *before* any other Phase 0 work. Migration files stay timestamp-named per the web `CLAUDE.md`.
...
| Concern | Package | Why |
|---|---|---|
| State/DI | `flutter_riverpod` (manual providers, no codegen — decided in plan 0C) | D4. Async + streams fit Supabase; overrideable repositories in tests (matches the current fake-repository pattern) |
...
| API client | `dio` + hand-written `ApiClient` in Phase 0 | 5 endpoints don't justify codegen; a contract test checks every used operationId against the pinned `api/openapi.json`. **Spike `swagger_parser` vs `openapi_generator` at the start of Phase 2**, when endpoints number in the dozens |
...
### 4.5 Design system
- Tokens from the web: background `#0B0B0F`, primary violet `#7C3AED`, accent text `#A78BFA`; Barlow Condensed (display) + Inter (body) as **bundled font assets, added in Phase 9 polish** (Phase 0 uses the palette with the default font); HSL token names from `tailwind.config.ts` ported to a `SxColors` extension (single source of truth, generated by script from the web tokens so they cannot drift).
...
- **S1–S3 (§2.5) fixed before public release — done 2026-09-19.**
...
|---|---|---|---|---|
| **0 Foundation & security** — *implemented by three plans: `2026-09-18-mobile-phase0a-security-hardening.md` (web), `…phase0b-api-foundation.md` (web), `…phase0c-flutter-foundation.md` (this repo)* | **Order matters:** (1) **S1–S3 first** (plan 0A): expand → contract on production with a rolled-back role-switching verification script and a prepared rollback, each production write approved by the owner — *independent of the rest of Phase 0*; (2) staging-DB decision deferred to before Phase 2; (4) flavors, Riverpod migration of existing slice, theme/tokens, i18n scaffold, api client pipeline (`openapi.json` → Dart), router w/ guards + link resolver, `/config`, Sentry + `/errors`, CI; (5) **Web:** bearer-auth helper, API scaffolding, `assetlinks.json`. | Yes (auth helper, `/config`, `/errors`, security migrations) | S1–S3 closed and verified by the rolled-back script, web smoke-tested; an authenticated round-trip through a bearer endpoint from a device; CI green; existing slice unchanged | L |
...
|---|---|---|
| S1–S3 security gaps (§2.5) | PII leak, score/KYC tampering, possible payment bypass | ✅ Closed 2026-09-19 — fixed and verified on production (no staging branch existed; see §2.5) |
...
3. **API client generator:** `swagger_parser`+retrofit vs `openapi_generator` — decided by a spike at the start of Phase 2 (moved from Phase 0), criteria: freezed output, null-safety fidelity, error envelope support.
