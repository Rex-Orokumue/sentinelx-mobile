...
- **Deferred from the master spec (with reason):** Dart codegen from OpenAPI (5 endpoints: hand-written client + a contract test is smaller; revisit at Phase 2 when endpoints number in the dozens), `riverpod_generator`, bundled brand fonts (Barlow Condensed/Inter — asset licensing/download is a Phase 9 polish item; Phase 0 theme uses tokens with the default font), the `pcm` locale (Flutter's Material delegates do not support `pcm`; needs a fallback delegate, done with the first real translated screens in Phase 1), Sentry (crashes go to `client_error_logs` first), feature-first file moves of the existing slice.
